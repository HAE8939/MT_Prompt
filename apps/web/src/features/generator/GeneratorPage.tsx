...
type Contribution = { sourceType: "TEMPLATE" | "SKILL" | "PERSONAL_RULE"; sourceKey: string; version: number; section: string; contentZh: string; contentEn: string };
type Compilation = { id: string; contentZh: string; contentEn: string | null; translationStatus: string; rulesSnapshot?: { contributions?: Contribution[] } };
type AiProposal = { contentZh: string; contentEn: string | null; notes: string[] };
...
export function GeneratorPage() {
  ...
  const saveMutation = useMutation({ mutationFn: () => api<{ id: string; title: string }>(`/compilations/${compileMutation.data?.id}/save-as-prompt`, { method: "POST", body: JSON.stringify({ title: requirement.trim().slice(0, 160) }) }), onSuccess: (prompt) => setSavedPromptId(prompt.id) });
  ...
  const aiMutation = useMutation({ mutationFn: (operation: "OPTIMIZE" | "VARIANTS" | "CONSISTENCY" | "REWRITE") => api<AiProposal>("/ai/assist", { method: "POST", body: JSON.stringify({ operation, contentZh: result?.contentZh, contentEn: result?.contentEn ?? undefined, targetModel: selectedModel?.name }) }) });
  const saveProposalMutation = useMutation({ mutationFn: () => api(`/prompts/${savedPromptId}/ai-proposals`, { method: "POST", body: JSON.stringify({ contentZh: aiMutation.data?.contentZh, contentEn: aiMutation.data?.contentEn }) }) });
  ...
  return <main className="generator-page">
    ...
    {selectedModel && selectedTask && selectedTemplate ? <div className="generator-layout">
      <section className="generator-form" aria-label="生成配置">
        ...
        <button className="primary-button generator-submit" disabled={!requirement.trim() || compileMutation.isPending} onClick={() => { saveMutation.reset(); retryMutation.reset(); aiMutation.reset(); saveProposalMutation.reset(); setSavedPromptId(null); compileMutation.mutate(); }}><Sparkles size={16} />{compileMutation.isPending ? "正在生成" : "生成 Prompt"}</button>
        {aiMutation.data && savedPromptId ? <button className="secondary-button" disabled={saveProposalMutation.isPending || saveProposalMutation.isSuccess} onClick={() => saveProposalMutation.mutate()}>{saveProposalMutation.isSuccess ? "建议已保存为新版本" : "保存 AI 建议为新版本"}</button> : null}
        {compileMutation.isError ? <p className="form-error">{compileMutation.error.message}</p> : null}
      </section>
      <section className="generator-result" aria-live="polite"><header><h2>生成结果</h2>{result ? <span className="result-status">{result.translationStatus === "SUCCEEDED" ? "双语完成" : "等待翻译"}</span> : null}</header>{result ? <div className="result-content"><PromptResult title="中文 Prompt" value={result.contentZh} language="zh" copied={copied} onCopy={copy} /><PromptResult title="English Prompt" value={result.contentEn} language="en" copied={copied} onCopy={copy} />{result.rulesSnapshot?.contributions?.length ? <section className="contribution-list"><h3>编译来源</h3>{result.rulesSnapshot.contributions.map((item, index) => <div key={`${item.sourceType}-${item.sourceKey}-${index}`}><strong>{item.sourceType}</strong><span>{item.sourceKey} · v{item.version} · {item.section}</span></div>)}</section> : null}<div className="ai-actions"><button onClick={() => aiMutation.mutate("OPTIMIZE")}>AI 优化</button><button onClick={() => aiMutation.mutate("VARIANTS")}>生成变体</button><button onClick={() => aiMutation.mutate("CONSISTENCY")}>一致性检查</button><button onClick={() => aiMutation.mutate("REWRITE")}>模型重写</button></div>{aiMutation.data ? <section className="ai-proposal"><h3>AI 建议（未保存）</h3><PromptResult title="建议中文 Prompt" value={aiMutation.data.contentZh} language="zh" copied={copied} onCopy={copy} />{aiMutation.data.notes.map((note) => <p key={note}>{note}</p>)}</section> : null}{aiMutation.isError ? <p className="form-error">{aiMutation.error.message}</p> : null}<div className="result-actions"><button className="primary-button" disabled={saveMutation.isPending || saveMutation.isSuccess} onClick={() => saveMutation.mutate()}><Save size={15} />{saveMutation.isPending ? "保存中..." : saveMutation.isSuccess ? "已保存到 Prompt 库" : "保存到 Prompt 库"}</button>{result.translationStatus === "FAILED" ? <button className="secondary-button" disabled={retryMutation.isPending} onClick={() => retryMutation.mutate()}><RefreshCw size={15} />{retryMutation.isPending ? "重试中..." : "重试翻译"}</button> : null}{saveMutation.isError || retryMutation.isError ? <p className="form-error">{(saveMutation.error ?? retryMutation.error)?.message}</p> : null}</div></div> : <div className="state-panel"><Sparkles size={28} /><h2>等待生成</h2><p>填写中文任务要求后，生成符合当前模型的双语 Prompt。</p></div>}</section>
    </div> : null}
  </main>;
}
