...
export function SettingsPage() {
  ...
  function setInterface(patch: Partial<InterfaceSettings>) {
    void updateSettings({ ...ui, ...patch });
  }
  ...
  return <main className="settings-page"><header className="page-header"><div><h1>设置</h1><span className="count">当前浏览器的 Provider 与界面偏好</span></div></header>
    ...
    <section className="settings-section"><header><div className="section-icon"><Palette size={17} /></div><div><h2>界面</h2><p>这些设置可选择随 .prompt 分享，Provider 永远不会包含。</p></div></header><div className="settings-form">
      <label>主题<select value={ui.theme} onChange={(event) => setInterface({ theme: event.target.value as InterfaceSettings["theme"] })}><option value="system">跟随系统</option><option value="light">浅色</option><option value="dark">深色</option></select></label>
      <label>Prompt 库默认视图<select value={ui.libraryView} onChange={(event) => setInterface({ libraryView: event.target.value as InterfaceSettings["libraryView"] })}><option value="list">列表</option><option value="grid">网格</option></select></label>
      <label className="enabled-toggle"><input type="checkbox" checked={ui.compact} onChange={(event) => setInterface({ compact: event.target.checked })} />紧凑模式</label>
    </div></section>
