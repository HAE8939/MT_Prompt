...
export function PromptEditor({ prompt, onClose, onSaved }: { prompt?: PromptRecord; onClose(): void; onSaved(updated: PromptRecord): void }) {
  const models = useQuery({ queryKey: ["models"], queryFn: () => api<Model[]>("/models") });
  const firstTask = models.data?.[0]?.tasks[0]?.id ?? "";
  const [title, setTitle] = useState(prompt?.title ?? "");
  const [contentZh, setContentZh] = useState(prompt?.contentZh ?? "");
  const [modelTaskId, setModelTaskId] = useState(prompt?.task.id ?? "");
  const mutation = useMutation({
    mutationFn: () => api<PromptRecord>(prompt ? `/prompts/${prompt.id}` : "/prompts", { method: prompt ? "PATCH" : "POST", body: JSON.stringify({ title, contentZh, modelTaskId: modelTaskId || firstTask }) }),
    onSuccess: onSaved,
  });

  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target) onClose(); }}>
    <section className="modal" role="dialog" aria-modal="true" aria-labelledby="new-prompt-title">
      <header><div><h2 id="new-prompt-title">{prompt ? "编辑 Prompt" : "新建 Prompt"}</h2><p>{prompt ? "修改后会生成新的版本记录。" : "先保存核心内容，图片和标签可在详情中补充。"}</p></div><button className="icon-button" onClick={onClose} aria-label="关闭"><X size={16} /></button></header>
      <form onSubmit={(event) => { event.preventDefault(); mutation.mutate(); }}>
        ...
        <footer><button type="button" className="secondary-button" onClick={onClose}>取消</button><button className="primary-button" disabled={mutation.isPending || !firstTask}>{mutation.isPending ? "保存中..." : prompt ? "保存修改" : "保存 Prompt"}</button></footer>
      </form>
    </section>
  </div>;
}
