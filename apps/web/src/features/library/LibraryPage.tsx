...
function DetailPanel({ prompt, onClose }: { prompt: PromptRecord; onClose(): void }) {
  ...
  return <aside className="detail-panel" aria-label="Prompt 详情">
    <header className="detail-header"><span>Prompt 详情</span><button className="icon-button" onClick={onClose} aria-label="关闭详情"><X size={16} /></button></header>
    <div className="detail-scroll">
      <div className="detail-visual"><div className="cover-placeholder large"><Image size={32} /><span>{prompt.model.mediaType}</span></div></div>
...
export function LibraryPage() {
  ...
  const query = useQuery({
    queryKey: ["prompts", deferredSearch],
    queryFn: () => api<PromptListResponse>(`/prompts?keyword=${encodeURIComponent(deferredSearch)}&page=1&limit=40`),
  });
  ...
  return <div className={`library-layout${selected ? " with-detail" : ""}`}>
    <main className="library-main">
      ...
      <div className="filter-bar">
        <div className="segments"><button className="active">全部</button><button>图片</button><button>视频</button><button>收藏</button></div>
      ...
      {query.data?.data.length === 0 ? <div className="state-panel empty"><Image size={32} /><h2>{search ? "没有匹配结果" : "Prompt 库还是空的"}</h2><p>{search ? "尝试缩短关键词或清除筛选。" : "新建第一个 Prompt，或从生成器保存结果。"}</p></div> : null}
      {query.data?.data.length ? <div className={view === "grid" ? "prompt-grid" : "prompt-list"}>{query.data.data.map((prompt) => <PromptCard key={prompt.id} prompt={prompt} selected={selected?.id === prompt.id} onOpen={() => setSelected(prompt)} />)}</div> : null}
    </main>
    {selected ? <DetailPanel prompt={selected} onClose={() => setSelected(null)} /> : null}
    {creating ? <PromptEditor onClose={() => setCreating(false)} onCreated={(prompt) => { setCreating(false); setSelected(prompt); void queryClient.invalidateQueries({ queryKey: ["prompts"] }); }} /> : null}
  </div>;
}
